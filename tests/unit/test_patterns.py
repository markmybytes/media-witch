...
class TestEpisodePatternProperties:
    ...
    @given(st.integers(min_value=1, max_value=99), st.integers(min_value=1, max_value=999))
    def test_standard_sXXeXX_format_always_matches(self, season: int, episode: int) -> None:
        ...
        assert has_episode_pattern(f'Show.S{season:02d}E{episode:02d}.mkv') is True
        assert has_episode_pattern(f'Show.S{season:02d}E{episode:03d}.mkv') is True
    ...
    def test_bracket_format_matches(self, episode: int) -> None:
        ...
        assert has_episode_pattern(f'Show.Name.[{episode:03d}].720p.mkv') is True
...
class TestNaturalSortProperties:
    ...
    @given(
        st.lists(st.integers(min_value=1, max_value=10000), min_size=2, max_size=50, unique=True)
    )
    ...
    def test_sort_is_case_insensitive(self, texts: list[str]) -> None:
        ...
        assert [p.name.lower() for p in sorted_original] == [p.name.lower() for p in sorted_lower]

    @given(
        st.text(alphabet='abcdefghijklmnopqrstuvwxyz', min_size=1, max_size=20),
        st.lists(st.integers(min_value=1, max_value=10000), min_size=2, max_size=20, unique=True),
    )
    def test_episodes_sort_numerically_not_lexically(
        ...
        sorted_episode_nums = [int(p.name.split('E')[1].split('.')[0]) for p in sorted_paths]

        assert sorted_episode_nums == sorted(episodes)

    @given(st.lists(st.integers(min_value=1, max_value=50), min_size=2, max_size=20, unique=True))
    ...
    @given(
        st.lists(
            st.tuples(st.integers(1, 20), st.integers(1, 50)), min_size=2, max_size=30, unique=True
        )
    )
    ...
    @given(
        st.lists(st.text(alphabet='0123456789', min_size=1, max_size=10), min_size=2, max_size=30)
    )
