...
def print_torrent_files(torrent: Dict[bytes, Any], torrent_filename: str) -> None:
    ...
    if b'files' in info:
        ...
        for file_dict in info[b'files']:
            path = '/'.join(p.decode('utf-8', errors='replace')
...
def create_fake_files(torrent_path: str, output_dir: str = None) -> None:
    ...
    if b'files' in info:  # Multi-file torrent
        for file_dict in info[b'files']:
            path_parts = [p.decode('utf-8', errors='replace')
                          for p in file_dict[b'path']]
            ...
            print(
                f"  Created: {os.path.relpath(full_path, base_dir)}  (0 bytes)")
            created_count += 1
    ...
    print(
        f"Done! {created_count} empty fake file(s) created in folder: {folder_name}\n")
...
def main():
    ...
    parser.add_argument(
        "path",
        help="Path to .torrent file or folder containing .torrent files"
    )
    parser.add_argument(
        "-o", "--output-dir",
        default=os.getcwd(),
    ...
    if os.path.isfile(args.path):
        if not args.path.lower().endswith('.torrent'):
            print(
                f"Error: File must be a .torrent file → {args.path}", file=sys.stderr)
            sys.exit(1)
        ...
        if not torrent_files:
            print(
                f"Error: No .torrent files found in folder: {args.path}", file=sys.stderr)
            sys.exit(1)
